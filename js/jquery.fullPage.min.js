...
function exTickerLoop(){
	var exWidths = Array();
	var counters = Array();
	counter = 0;

	$(".scrolling-text-wrapper").each(function() {				
		exWidths.push($(this).find(".scrolling-text").eq(0).width());
		counters.push(0);
	});
	
	var scrollingtimer = setInterval(function(){
		$(".scrolling-text-wrapper").each(function() {
			var current = $(".scrolling-text-wrapper").index($(this));
			var speed = parseFloat($(this).data("count"));
			var directon = $(this).data("dir");
			if (counters[current] >= exWidths[current]*10) {
				counters[current] = 0;
			}
			else {					
				counters[current] += speed;
			}
			if (directon == "pos") {
				$(this).css("-moz-transform", "translate(-"+counters[current]+"px, 0)");
				$(this).css("-o-transform", "translate(-"+counters[current]+"px, 0)");
				$(this).css("-ms-transform", "translate(-"+counters[current]+"px, 0)");
				$(this).css("-webkit-transform", "translate(-"+counters[current]+"px, 0)");
				$(this).css("transform", "translate(-"+counters[current]+"px, 0)");
			} else {
				if (counters[current] >= window.innerWidth*5) {
					counters[current] = 0;
				}

				var nlimit = $(this).data("ntotal");
				if (nlimit != undefined) {
					var width = ($(this).find(".scrolling-text").eq(0).width()-nlimit)*10;
				} else {
					var width = ($(this).find(".scrolling-text").width())*5;
				...
				$(this).css("-ms-transform", "translate(-"+counters[current]+"px, 0)");
				$(this).css("-webkit-transform", "translate(-"+counters[current]+"px, 0)");
				$(this).css("transform", "translate(-"+(width - counters[current])+"px, 0)");
			}				
		});
	}, 3);
}

function populateExTicker() {
	$(".scrolling-text-wrapper").each(function() {		
		for (var i = 0; i < 20; i++) {
			clone = $(this).find(".scrolling-text").eq(0).clone();
			clone.appendTo($(this));
		}
		var directon = $(this).data("dir");
		var ntotal = $(this).data("ntotal");
		var fullWidth = $(this).find(".scrolling-text").eq(0).width()*5;
		fullWidth = window.innerWidth * 5;

		if (directon == "neg"){
			$(this).css("-moz-transform", "translate(-"+fullWidth+"px, 0)");
			$(this).css("-o-transform", "translate(-"+fullWidth+"px, 0)");
			$(this).css("-ms-transform", "translate(-"+fullWidth+"px, 0)");
			$(this).css("-webkit-transform", "translate(-"+fullWidth+"px, 0)");
			$(this).css("transform", "translate(-"+fullWidth+"px, 0)");
		}
	});
	exTickerLoop();
}
